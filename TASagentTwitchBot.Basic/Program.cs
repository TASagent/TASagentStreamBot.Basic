...
BGC.IO.DataManagement.Initialize("TASagentStreamBotBasic");
...
mvcBuilder.RegisterControllersWithoutFeatures("Midi", "Scripting");

//Add SignalR for Hubs
builder.Services.AddSignalR();

//Custom Database
builder.Services.AddTASDbContext<TASagentTwitchBot.Basic.Database.DatabaseContext>();
...
//Custom Notification
builder.Services
    .AddTASSingleton<TASagentTwitchBot.Basic.Notifications.TTSOnlyActivityProvider>();
...
builder.Services
    ...
    .AddTASSingleton<TASagentTwitchBot.Core.TTS.TTSRedemptionHandler>()
...
{
    TASagentTwitchBot.Basic.Database.DatabaseContext context = serviceScope.ServiceProvider!.GetRequiredService<TASagentTwitchBot.Basic.Database.DatabaseContext>();
...
TASagentTwitchBot.Core.TTS.TTSConfiguration GetDefaultTTSConfig() =>
    new TASagentTwitchBot.Core.TTS.TTSConfiguration()
    {
        Version = TASagentTwitchBot.Core.TTS.TTSConfiguration.CURRENT_VERSION,
        OverrideVoices = true,
        ...
        Redemption = new TASagentTwitchBot.Core.TTS.TTSConfiguration.RedemptionConfiguration()
        {
            Enabled = true,
            Paused = false
        },
        ModPermissions = new TASagentTwitchBot.Core.TTS.TTSConfiguration.CommandRolePermissions()
        {
            CanUseCommand = TASagentTwitchBot.Core.TTS.TTSConfiguration.Permission.Never,
            CanUseRedemption = TASagentTwitchBot.Core.TTS.TTSConfiguration.Permission.Always,
            CanCustomize = false,
            AllowNeuralVoices = true
        },
        ElevatedPermissions = new TASagentTwitchBot.Core.TTS.TTSConfiguration.CommandRolePermissions()
        {
            CanUseCommand = TASagentTwitchBot.Core.TTS.TTSConfiguration.Permission.Never,
            CanUseRedemption = TASagentTwitchBot.Core.TTS.TTSConfiguration.Permission.Always,
            CanCustomize = false,
        ...
        RiffRaffPermissions = new TASagentTwitchBot.Core.TTS.TTSConfiguration.CommandRolePermissions()
        {
            CanUseCommand = TASagentTwitchBot.Core.TTS.TTSConfiguration.Permission.Never,
            CanUseRedemption = TASagentTwitchBot.Core.TTS.TTSConfiguration.Permission.WithApproval,
            CanCustomize = false,
            AllowNeuralVoices = true
        }
    };
